...
choices1.push(new Choice("Ask for more information like source name and org name", 2, 2, 0, 0));
...
choices2.push(new Choice("Ask the client why is the document secured instead of public", 2, 4, 10, 0));
choices2.push(new Choice("Ask what identity/email they have in their search", 2, 4, 0, 0));
choices2.push(new Choice("Ask if the user should have access or not in Salesforce", 2, 4, 5, 0));
choices3.push(new Choice("Ask client for a delegated access", 3, 5, 0, 1));
choices3.push(new Choice("Tell client to change the filter on their page", 3, 5, 10, 1));
choices3.push(new Choice("Even if the user should have access to the document it might be filtered out ", 3, 5, 5, 1));
choices3.push(new Choice("Anonymous user is not a real user hence why you are not seeing the document", 0, 8, 0, 1));
choices4.push(new Choice("Send email of the modification to be done", 4, 9, 10, 2));
choices4.push(new Choice("Go on meeting with customer to fix the filter with them", 4, 10, 0, 2));
choices4.push(new Choice("Send KB to the client about the modification that should be done", 4, 11, 5, 2));
choices4.push(new Choice("You do modification directly in their prod", 4, 12, 0, 2));
