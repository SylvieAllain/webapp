...
var choices3 = [];
var choices4 = [];
choices1.push(new Choice("Open customer environment and look at the source", 0, 1, 5));
choices1.push(new Choice("Ask for more information like source name and org name", 1, 2, 0));
choices1.push(new Choice("Send a link from google to the super deflection KB", 0, 6, 600)); //might have to change the next context if the 2 context in comments arent comments anymore
choices1.push(new Choice("Let the client know the next refresh should have it.", 0, 3, 10));
choices2.push(new Choice("Tell client the document is present and you can see it, no issue", 0, 7, 0));
choices2.push(new Choice("Ask the client why is the document secured instead of public", 1, 2, 10));
choices2.push(new Choice("Ask what identity/email they have in their search", 2, 4, 0));
choices2.push(new Choice("Ask if the user should have access or not in Salesforce", 1, 2, 5));
choices3.push(new Choice("Ask client for a delegated access", 3, 5, 0));
choices3.push(new Choice("Tell client to change the filter on their page", 2, 4, 10));
choices3.push(new Choice("Even if the user should have access to the document it might be filtered out ", 2, 4, 5));
choices3.push(new Choice("Anonymous user is not a real user hence why you are not seeing the document", 0, 8, 0));
choices4.push(new Choice("Send email of the modification to be done", 0, 9, 10));
choices4.push(new Choice("Go on meeting with customer to fix the filter with them", 0, 10, 0));
choices4.push(new Choice("Send KB to the client about the modification that should be done", 0, 11, 5));
choices4.push(new Choice("You do modification directly in their prod", 0, 12, 0));
...
context.push(new Context("You didn't solve the issue and the custumer is unsatisfied", false, true));
context.push(new Context("The issue is not solved and the client is angry", false, true));
context.push(new Context("The issue is solved but the client would of liked to have a personal interation wiht you.", false, true));
context.push(new Context("The issue is solved and the client is really happy that you took the time to personally meet with them and guide them throught the process.", false, true));
