...
choices1.push(new Choice("Ask for more information such as which source and for how long this has hapened", 2, 2, 0));
...
choices3.push(new Choice("Look at the log browser in their organizationl", 2, 9, 10));
...
choices5.push(new Choice("Tell the customer that it was a temporary issue. Cancel the current Rebuild or Resume it's operation to resolve he issue", 5, 16, 0));
choices5.push(new Choice("See if there were any planned maintenance for Salesforce at the specified time since the 503 error hapened on a Salesforce source.", 6, 17, 0));
choices5.push(new Choice("You look into the 503 Issue with the R&D team", 4, 18, 0));

choices6.push(new Choice("Investigate the 503 Error with the R&D Team.", 5, 19, 0));
choices6.push(new Choice("Explain the 503 Error code to the customer and what it means.", 5, 20, 0));
choices6.push(new Choice("Look at the Salesforce Status and Maintenance page to confirm if the server could have been unavailable", 6, 21, 0));

choices7.push(new Choice("Tell the customer that the issue hapened due to a Salesforce Maintenance. Provide the link to the known issue. Then tell him he needs to either resume the current operation or cancel it and start a new one.", 6, 22, 0));
choices7.push(new Choice("Tell the customer that the issue hapened due to a Salesforce Maintenance. Provide the link to the known issue.  Then tell him that you've Canceled the current operation and started a Rebuild.", 6, 23, 10));
choices7.push(new Choice("Tell the customer that the issue hapened due to a Salesforce Maintenance. Provide the link to the known issue. Then tell the customer to start a Rebuild.", 6, 24, 5));
...
context.push(new Context("The Customer tells you that a shcedule is already set on the source.", false, false));
...
context.push(new Context("Nothing happens, the source state doesn't change and remains 'Paused on Error'.", false, false));
//context 4
context.push(new Context("Both issue were related so you'll do double the work for nothing. But lets keep going.", false, false));
...
context.push(new Context("Customer escalate the Issue into a hgiher one and escalate the case. You created work for the ops team which need to restore a backup and made a change you weren't allowed to.", false, true));
context.push(new Context("Both issue hapened on the same day within 1 minute of themselves and the Error Code in the Activity Browser is the same ERROR: 503 Service Unavailable.", false, false));
//context 5
...
context.push(new Context("You notice that Salesforce had a planned maintenance with disruption of service (Specifically SOQL and REST API services) at the date of the issue. You can now tell the customer what hapened.", false, false));
//ending
